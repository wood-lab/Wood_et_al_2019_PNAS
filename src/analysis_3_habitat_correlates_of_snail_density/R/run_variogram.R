...
run_variogram <- function( data, data_list, pred, bulinus, model){
  ...
  for(i in  1:nrow(site_time)){
    ...
    if(sum(is.na(dat_sub$lat)) == 0){
      coordinates(dat_sub) <- c("long", "lat")
      v = variogram(resid~1, dat_sub)
      dat_sub <- as.data.frame(dat_sub)

      filename <- paste("Report/Figures/Variograms/spatio_temporal_variogram_of_ ", dat_sub$site[1], "_field_mission_", dat_sub$field_mission[1], "_", c("biom", "bulinus")[as.numeric(bulinus)+1], "_model_", model,".png",sep = "")
  ...
  for(i in  1:length(site_time)){
    ...
    if(sum(is.na(dat_sub$lat)) == 0){
      ...
      filename <- paste("Report/Figures/Variograms/spatial_variogram_of_ ", dat_sub$site[1], "_", c("biom", "bulinus")[as.numeric(bulinus)+1], "_model_", model,".png",sep = "")
      png( file=filename, width=12, height=6, res=200, units='in')
      par(mfrow = c(1,2))
      plot(x = v$dist, y = v$gamma, main = paste0("Variogram of residuals: ", dat_sub$site[1]), xlab = "Distance (m)", ylab = "Semivariance")
      plot(x = dat_sub[,c_q], y = dat_sub$pred, main = paste0("Fitted vs observed:", dat_sub$site[1]), xlab = "Observed count", ylab = "Predicted count")
  ...
  coordinates(dat) <- c("site_long", "site_lat")
  v = variogram(resid~1, dat)#, cutoff = max(spDists(dat))/2)
  dat <- as.data.frame(dat)
  plot(v)
  filename <- paste("Report/Figures/Variograms/among_site_variogram_", c("biom", "bulinus")[as.numeric(bulinus)+1],"_model_", model,  ".png",sep = "")
  png( file=filename, width=12, height=6, res=200, units='in')
  par(mfrow = c(1,2))

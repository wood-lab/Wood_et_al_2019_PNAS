# Function to read the data used in the analysis. Also conducts preliminary data analysis on covariates. Returns a list of the FULL data to be used in TMB. Covariates include "lake_river" where lake = 1, river = 0.

read_dat <- function(pda = F, bulinus = T){

  # Step 0 -- Load data and packages
  library(lattice)
  library(readxl)

  density_data <- read.csv("data/Wood_et_al_Schisto_data_5_8_2018.csv", na.strings=c(""))
  ...
  density_temporal_names <- c("sampling_date", "field_mission", "sampling_start_time_cleaned")

...
build_data <- function(data, model, debug = 1, bulinus = T, parasite = 0){
  ...
  data_list$x_pq = as.matrix( cbind( density_data[,grep("cont_covar_", colnames(density_data))], density_data[,grep("cat_covar_", colnames(density_data))]))
  data_list$x_cq = as.matrix( cbind( density_data[,grep("cont_covar_", colnames(density_data))], density_data[,grep("cat_covar_", colnames(density_data))]))
  ...
  # Index missing data
  data_list$n_x_pq_cont <- length(grep("cont_covar_", colnames(data_list$x_pq)))
  data_list$n_x_cq_cont <- length(grep("cont_covar_", colnames(data_list$x_cq)))

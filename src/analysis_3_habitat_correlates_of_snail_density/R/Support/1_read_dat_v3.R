...
read_dat <- function(exclude = T, fill_cont_mean = F, na_keep = T, quad = F, scale_cont = T, fill_cat_mode = F, bulinus = T, remove_outlier = T){
  ...
  continuous_density_covariate_names <- c("emergent_stems_cleaned", "floating_veg_g_cleaned", "secchi_depth_cm", "pH", "nitrate_mg.L_NO3", "nitrite_mg.L_NO2", "phosphate_mg.L_PO4", "depth_cm_cleaned_agg") # No longer include = "oxygen_mg.L_o2", "salinity_brix", "flowmeter_rotations", "temp_C"
  ...
  if( exclude == T ){
    #individual_data$include_in_which_analyses <- merge(individual_data,  density_data, by = quad_ids, all.x = T)$include_in_which_analyses
    density_data <- density_data[-which(density_data$include_in_which_analyses == "exclude"),]
    # individual_data <- individual_data[-which(individual_data$include_in_which_analyses == "exclude"),]
  }
...
# Interaction term
  density_data$cont_covar_floating_vs_emergent <- density_data$cont_covar_floating_veg_g_cleaned * density_data$cont_covar_emergent_stems_cleaned
  continuous_density_covariate_names <- c(continuous_density_covariate_names, "cont_covar_floating_vs_emergent")


  # Data summary
  names <- c(continuous_density_covariate_names, categorical_density_covariate_names)
  percent_na <- c(colSums(is.na(density_data[,continuous_density_covariate_names])), colSums(is.na(density_data[,categorical_density_covariate_names]))) / nrow(density_data)
  mean_mode <- c(apply(density_data[,continuous_density_covariate_names],2, mean, na.rm = T), apply(density_data[,categorical_density_covariate_names],2, Mode, na.rm = T))
  sd_vals <- c(apply(density_data[,continuous_density_covariate_names],2, sd, na.rm = T), rep(NA, length(categorical_density_covariate_names)))
  ...
  if(fill_cat_mode == T){
    for(i in 1:length(categorical_density_covariate_names)){
      ...
      density_data[which(is.na(density_data[,categorical_density_covariate_names[i]])),categorical_density_covariate_names[i]] <- Mode(density_data[which(!is.na(density_data[,categorical_density_covariate_names[i]])),categorical_density_covariate_names[i]] )
    }
  }
  ...
  if(na_keep == F){
    ...
    #individual_data <- individual_data[complete.cases(individual_data[,continuous_density_covariate_names]),]

    density_data <- density_data[complete.cases(density_data[,categorical_density_covariate_names]),]
    #individual_data <- individual_data[complete.cases(individual_data[,categorical_density_covariate_names]),]
  }




  #individual_data[,continuous_density_covariate_names[i]] <- as.numeric(as.character(individual_data[,continuous_density_covariate_names[i]]))

  # Step - 9 Scale continuous covariates
  for(i in 1:length(continuous_density_covariate_names)){
    # Scale
    if(scale_cont == T){
      density_data[,continuous_density_covariate_names[i]] <- scale(density_data[,continuous_density_covariate_names[i]])
    }

    #individual_data[,continuous_density_covariate_names[i]] <- scale(individual_data[,continuous_density_covariate_names[i]])

    # Fill with mean (i.e. 0)
    if(fill_cont_mean == T){
      density_data[which(is.na(density_data[,continuous_density_covariate_names[i]])),continuous_density_covariate_names[i]] <- median(density_data[,continuous_density_covariate_names[i]], na.rm = T)
      #individual_data[which(is.na(individual_data[,continuous_density_covariate_names[i]])),continuous_density_covariate_names[i]] <- 0
    }
  }

  if(remove_outlier){
    density_data <- density_data[which(as.numeric(as.character(density_data$snail_is_trunc_globo)) < 900),]
  }

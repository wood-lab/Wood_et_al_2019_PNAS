...
map_update <- function( map , params , data_list ){
  ...
  map$x_q_cont_missing[,which(1:ncol(map$x_q_cont_missing) %in% which(is.na(beta_p) & is.na(beta_c)))] <- NA # Don't estimate data for the parameters that are turned off

...
#' @export
Optimize_optim = function( obj, fn=obj$fn, gr=obj$gr, startpar=obj$par, lower=rep(-Inf,length(startpar)), upper=rep(Inf,length(startpar)),
                           getsd=TRUE, method = "Nelder-Mead", control=list(maxit=1e4,trace=0), bias.correct=FALSE,
  ...
  start_time = Sys.time()
  if(method == "nlminb"){
    parameter_estimates = suppressWarnings(nlminb( start=startpar, objective=fn, gradient=gr, control=control, lower=lower, upper=upper ))
  } else {
    parameter_estimates = suppressWarnings(optim( par=startpar, fn=fn, gr=gr, control=control, lower=lower, upper=upper, method = method))
  }
  ...
  for( i in seq(2,loopnum,length=max(0,loopnum-1)) ){
    if(method == "nlminb"){
      parameter_estimates = suppressWarnings(nlminb( start=parameter_estimates$par, objective=fn, gradient=gr, control=control, lower=lower, upper=upper ))
    } else{
      parameter_estimates = suppressWarnings(optim( par=parameter_estimates$par, fn=fn, gr=gr, control=control, lower=lower, upper=upper, method = method))
    }
  }
  ...
  for(i in seq_len(newtonsteps)) {
    ...
    parameter_estimates$value <- fn(parameter_estimates$par)
  }

  # Exclude difficult-to-interpret messages
  parameter_estimates = parameter_estimates[c('par','value')]
